...
export default function ACPChatPage() {
  ...
  const loadMeta = useCallback(async () => {
    setLoadingMeta(true);
    try {
      const [routeList, vkList] = await Promise.all([listACPRoutes(), listVirtualKeys()]);
      const activeRoutes = routeList.filter((r) => !r.disabled);
      setRoutes(activeRoutes);
      setVirtualKeys(vkList);
      setSelectedRouteId((prev) => prev || activeRoutes[0]?.id || "");
    } catch (err) {
      showToast(err instanceof ApiError ? err.message : "Failed to load chat metadata", "error");
    } finally {
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
          <div className="min-w-0 flex-1">
            ...
            <select
              ...
              disabled={loadingMeta || streaming}
              onChange={(e) => {
          ...
          <div className="shrink-0">
            <Button variant="secondary" onClick={startNewChat} disabled={streaming}>
              New chat
            </Button>
          </div>
        </div>
